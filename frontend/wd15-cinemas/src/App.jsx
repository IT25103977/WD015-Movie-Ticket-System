...
import { apiGet, apiPost, normalizeMovie } from './api';
import {
...
const Navbar = ({ onOpenLogin, user, onLogout, setView, onOpenAdmin }) => (
  <nav className="fixed top-0 left-0 right-0 z-[60] bg-[#121212]/95 backdrop-blur-md border-b border-white/10">
    <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
      ...
      <div className="hidden lg:flex items-right gap-8 text-[16px] font-black tracking-[0.2em] text-gray-500 uppercase">
        ...
        <button onClick={onOpenAdmin} className="hover:text-white">
          Admin
        </button>
      </div>
...
export default function App() {
  const [users, setUsers] = useState([
    { id: 1, name: "Admin", email: "admin@gmail.com", password: "admin123", role: "Admin", status: "Active", joinedDate: "2026-05-16", lastLogin: "Just now" },
    { id: 2, name: "Tharusha", email: "user@gmail.com", password: "user123", role: "Customer", status: "Active", joinedDate: "2026-05-16", lastLogin: "Just now" }
  ]);
  ...
  useEffect(() => {
    ...
    apiGet('/cinemas')
      .then(data => setCinemas(data.map(c => typeof c === 'string' ? c : c.name)))
    ...
    apiGet('/times')
      .then(data => setShowtimes(data.map(t => t.slotValue)))
  ...
  const sameBookingSlot = (item) => {
    const movieId = item.show?.movie?.id || item.movieId;
    const cinemaName = item.show?.cinema?.name || item.cinemaName;
    const showDate = item.show?.showDate || item.date;
    const showTime = item.show?.showTime || item.time;
    return movieId === selectedMovie?.id && cinemaName === booking.cinema && showDate === booking.date && showTime === booking.time;
  };

  const bookedSeats = useMemo(() => {
    if (!selectedMovie) return [];
    return bookings.filter(sameBookingSlot).flatMap(item => item.seats || []);
  }, [bookings, selectedMovie, booking.cinema, booking.date, booking.time]);

  const startBooking = (movie) => {
    if (movie.status === 'Coming Soon') return;
    setSelectedMovie(movie);
    setBooking({ ...booking, adults: 1, children: 0, seats: [], food: [], cinema: (typeof cinemas[0] === 'string' ? cinemas[0] : cinemas[0]?.name) || CINEMAS[0], time: '' });
    setBookingStep(1);
    setView('booking');
  };
  ...
  const handleLogin = () => {
    apiPost('/users/login', loginForm)
      ...
      .catch(() => {
        const localUser = users.find(u => (u.email === loginForm.email || u.name === loginForm.email) && u.password === loginForm.password);
        if (localUser) {
          setUser(localUser);
          setIsAdmin(localUser.role === 'Admin');
          setIsLoginOpen(false);
        } else {
          alert('Wrong email/name or password');
        }
      });
  };

  const handleSignup = () => {
    if (!signupForm.name || !signupForm.email || !signupForm.password) return alert('Please fill all signup fields');
    const draft = {
      ...signupForm,
      role: 'Customer',
      status: 'Active',
      ...
      lastLogin: 'Just now',
      avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + encodeURIComponent(signupForm.name)
    };
    apiPost('/users/signup', draft)
      ...
      .catch(() => {
        const localUser = { ...draft, id: Date.now() };
        setUser(localUser);
  ...
  const confirmBooking = () => {
    const matchingShow = shows.find(s => s.movie?.id === selectedMovie?.id && s.cinema?.name === booking.cinema && s.showDate === booking.date && s.showTime === booking.time)
      || shows.find(s => s.movie?.id === selectedMovie?.id)
      || shows[0];
    const bookingPayload = {
      showId: matchingShow?.id || 1,
      movieId: selectedMovie?.id,
      userId: user?.id,
      cinemaName: booking.cinema,
      date: booking.date,
      time: booking.time,
      ...
      payment: { method: 'Credit / Debit Card', status: 'PAID', amount: totals.total }
    };
    apiPost('/bookings', bookingPayload)
      .then(saved => {
        setBookings(prev => [...prev, saved]);
        setView('success');
      })
      .catch((err) => {
        console.error('Booking backend failed, saved locally:', err);
        setBookings(prev => [...prev, { ...bookingPayload, id: Date.now(), user, show: { movie: selectedMovie, cinema: { name: booking.cinema }, showDate: booking.date, showTime: booking.time } }]);
        setView('success');
      });
  };
              ...
              <div className="bg-[#1e1e1e] border border-white/10 rounded-2xl p-6 shadow-2xl flex flex-col md:flex-row items-end gap-6">
                <div className="flex-1 w-full space-y-2">
                  ...
                  <select className="w-full bg-[#121212] border border-white/5 text-white p-4 rounded-xl outline-none focus:border-[#e11d48] appearance-none" onChange={(e) => setBooking({ ...booking, cinema: e.target.value })}>
                    {cinemas.map((c, i) => {
                      const cinemaName = typeof c === 'string' ? c : c.name;
                      return <option key={cinemaName || i} value={cinemaName}>{cinemaName}</option>;
                    })}
                  </select>
                </div>
                <div className="flex-1 w-full space-y-2">
                    ...
                    <div className="bg-[#121212] p-12 rounded-[3rem] border border-white/5">
                      ...
                      <div className="grid grid-cols-10 gap-3 place-items-center">
                        {Array.from({ length: 40 }).map((_, i) => {
                          const id = `S${i + 1}`;
                          const isOccupied = bookedSeats.includes(id);
                ...
                <div className="lg:col-span-1">
                  <div className="sticky top-32 bg-[#1e1e1e] p-10 rounded-[3rem] border border-white/10 shadow-2xl space-y-8 animate-in slide-in-from-right">
                    ...
                    <div className="pt-8 border-t border-white/5">
                      ...
                      <button onClick={bookingStep === 6 ? confirmBooking : nextStep} className={`w-full py-5 rounded-2xl font-black text-lg tracking-widest uppercase shadow-xl transition-all active:scale-95 ${bookingStep === 6 ? 'bg-green-600' : 'bg-[#e11d48]'}`}>
      ...
      {isLoginOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1a1a1a] border border-white/10 w-full max-w-md rounded-[2rem] p-8">
            ...
            {authMode === "login" && (
              <>
                <input placeholder="Email or Name" value={loginForm.email} onChange={(e) => setLoginForm({ ...loginForm, email: e.target.value })} className="w-full mb-3 p-3 bg-black border border-white/10" />
                <input type="password" placeholder="Password" value={loginForm.password} onChange={(e) => setLoginForm({ ...loginForm, password: e.target.value })} className="w-full mb-4 p-3 bg-black border border-white/10" />
            ...
            {authMode === "signup" && (
              <>
                <input placeholder="Name" value={signupForm.name} onChange={(e) => setSignupForm({ ...signupForm, name: e.target.value })} className="w-full mb-3 p-3 bg-black border border-white/10" />
                <input placeholder="Email" value={signupForm.email} onChange={(e) => setSignupForm({ ...signupForm, email: e.target.value })} className="w-full mb-3 p-3 bg-black border border-white/10" />
                <input type="password" placeholder="Password" value={signupForm.password} onChange={(e) => setSignupForm({ ...signupForm, password: e.target.value })} className="w-full mb-4 p-3 bg-black border border-white/10" />
        ...
        <div className="max-w-[1440px] mx-auto">
          ...
          <div className="border-t border-white/10 pt-10 flex flex-col items-center gap-6 text-center">
            ...
            <p className="text-[10px] uppercase tracking-[0.4em] text-white/20 font-black">
              Â© 2026 WD15 CINEMAS. ALL RIGHTS RESERVED.
            </p>
